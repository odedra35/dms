...
app = Flask(__name__)
app.config.update(config['routes'])
logging.config.dictConfig(config['logger'])
logger = logging.getLogger(__name__)
# remove before upload
app.secret_key = config['routes']['SECRET_KEY']
URLS_LIMIT=config['routes']['URLS_LIMIT']
ERROR_LIMIT_PCT = config['routes']['ERROR_LIMIT_PCT']
TIME_LIMIT = config['routes']['TIME_LIMIT']
...
def check_single():
    ...
    if request.method == 'POST':
        ...
        logger.log(logger.level, f"r_op_h {radio_option_hours}\nr_op_f {radio_option_fixed}\nDisable {disable}\nhours {hours_input}\nfixed {fixed_input}")
        ...
        if hours_input or fixed_input:
            ...
            msg = add_user_scheduler(user, url, hours_input, fixed_input, URLS_LIMIT)
            if msg:
        ...
        # Normal single check
            ...
            logger.log(logger.level, f"{timed=} seconds")
...
def bulk_upload():
    ...
    if request.method == 'POST':
        ...
        if hours_input or fixed_input:
            ...
            # Make new (overwrite) scheduler for user
            msg = add_user_scheduler(user, url_file.stream.read(), hours_input, fixed_input, URLS_LIMIT = config['routes']['URLS_LIMIT'])
            if msg:
                return render_template('home.html', username=user, error=msg)
        ...
        # Normal Multi check
            ...
            logger.log(logger.level, f"{error_pct=}%")
            ...
            logger.log(logger.level, f"{timed=}")
            if timed > TIME_LIMIT:
